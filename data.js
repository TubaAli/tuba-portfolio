...
const SPRINT = {
  ...
  cases: [
    ...
    { n: 3, status: "live", slug: "ai-collaborator", page: "ai-collaborator.html", icon: "芽",
      ...
      solution: "A pair of personal agents — Olo 🌱 (mine) and AIJoe 🤖 (my professor's) — around a shared “outline” repo that is the project's single source of truth: GOALS, CONSTRAINTS, METHODS, DECISIONS, UNAGREED, TRACKING. Olo reads a thread, drafts a reply in my voice (VOICE.md, extracted from my real messages) and posts it as a Slack draft for my approval. Anything that would change an agreement is filed to UNAGREED.md and becomes a DECISION only after every member 👍s the same resolution. Protocol v2 made the whole thing a written, ratified rulebook: every proposed change gets its own approval thread with a proof link (👍👍 approves, 🚩 freezes), a four-section daily huddle replaces ping-storms, agents answer each other directly so nobody spams the professor, and one weekly sync stays humans-only. Agents propose and remind — humans decide.",
      ship: ["Claude Code agents", "Slack drafts — never auto-send", "PERSONA.md — five-element persona", "VOICE.md — my style, from real messages", "outline repo — 6 agreement files", "👍 human-only promotion protocol", "protocol v2 — the ratified agent rulebook", "daily huddle — Agreed / Remaining / For Tuba / For Joe"],
